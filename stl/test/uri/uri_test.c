...
void test11(void)
{
    struct stl_uri *uri = NULL;

    assert(stl_uri_create("127.0.0.1") == NULL);
    assert(stl_uri_create("") == NULL);
    assert(stl_uri_create("/dsad") == NULL);
    assert(stl_uri_create(NULL) == NULL);
    assert(stl_uri_create("http://127.0.0.1:") == NULL);
    assert(stl_uri_create("http://127.0.0.1:88888") == NULL);
    assert(stl_uri_create("ldap://[2001:db8::7/c=GB?objectClass?one") == NULL);
    assert(stl_uri_create("ldap://[2001:db8::7") == NULL);
...
void fail_test()
{
    struct stl_uri *uri;
    fail_malloc = true;
    assert(stl_uri_create("tcp://127.0.0.1") == NULL);
    fail_malloc = false;

    uri = stl_uri_create("tcp://127.0.0.1");
    assert(uri != NULL);
    stl_uri_destroy(uri);
    ...
    fail_strtoul = 1;
    assert(stl_uri_create("tcp://127.0.0.1:9000") == NULL);
    fail_strtoul = 2;
    assert(stl_uri_create("tcp://127.0.0.1:9000") == NULL);
    fail_strtoul = 0;
}
