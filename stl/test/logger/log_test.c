...
int callback(void *arg, stl_log_level level, const char *fmt, va_list va)
{
    (void)level;
    (void)fmt;
    (void)va;
...
void test1(void)
{
    ...
    stl_log_init();
    stl_log_set_callback(&count, callback);
    assert(stl_log_set_level("errrorr") == -1);
    assert(stl_log_set_level("errox") == -1);
    assert(stl_log_set_level("err") == -1);
    stl_log_debug("test \n");
    assert(count == 0);

    assert(stl_log_set_level("debuG") == 0);
    stl_log_set_level("DEBUG");
    stl_log_debug("test \n");
    assert(count == 1);
    stl_log_info("test \n");
    assert(count == 2);
    ...
    count = 0;
    assert(stl_log_set_level("iNfO") == 0);
    stl_log_set_level("INFO");
    stl_log_debug("test \n");
    assert(count == 0);
    stl_log_info("test \n");
    assert(count == 1);
    ...
    stl_log_set_level("INFO");
    stl_log_set_stdout(false);
    assert(stl_log_set_file("prev.txt", "current.txt") == 0);
    for (int i = 0; i < 100000; i++)
    {
        stl_log_error("testtesttesttesttesttesttesttesttesttesttesttest");
    }

    FILE *fp = fopen("prev.txt", "rb");
    assert(fp != NULL);
    fseek(fp, 0, SEEK_END);
    assert(ftell(fp) >= STL_LOG_FILE_SIZE);
    fclose(fp);
...
int callback2(void *arg, stl_log_level level, const char *fmt, va_list va)
{
    (void)arg;
    (void)level;
...
extern tm *__real_localtime_r(const time_t *timer, tm *res);
tm *__wrap_localtime_r(const time_t *timer, tm *res)
{
    if (!mock_localtime_r)
    {
...
void fail_test(void)
{
    ...
    stl_log_set_stdout(true);
    stl_log_set_file("tmp.txt", "tmp2.txt");
    stl_log_set_callback(NULL, callback2);

    fprintf_count = 0;
    vfprintf_count = 0;
    stl_log_info("loggg");
    assert(vfprintf_count + fprintf_count == 6);
    stl_log_set_callback(NULL, NULL);

    fprintf_count = 0;
    vfprintf_count = 0;
    stl_log_set_stdout(false);
    stl_log_set_file(NULL, NULL);
    stl_log_info("loggg");
    assert(vfprintf_count + fprintf_count == 0);

    stl_log_set_stdout(true);
    fprintf_ret = -1;
    assert(stl_log_info("test") == -1);
    fprintf_ret = 0;
    assert(stl_log_info("test") == 0);
    ...
    assert(stl_log_info("test") == 0);
    fprintf_ret = -1;
    assert(stl_log_set_file("tmp.txt", "tmp2.txt") == -1);
    fprintf_ret = 0;

    assert(stl_log_set_file(NULL, "test.txt") == 0);
    assert(stl_log_set_file("test.txt", NULL) == 0);
    mock_fopen = true;
    assert(stl_log_set_file("prev.txt", "current.txt") == -1);
    mock_fopen = false;
    mock_ftell = true;
    assert(stl_log_set_file("prev.txt", "current.txt") == -1);
    mock_ftell = false;
    ...
    {
        failed = stl_log_error("testtesttesttesttesttesttesttesttesttestest");
        if (failed < 0)
        {
            break;
        }
...
int main()
{
    stl_log_set_thread_name("My thread");
