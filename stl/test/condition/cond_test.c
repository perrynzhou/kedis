...
void stl_thread_init(struct stl_thread *thread);
int stl_thread_term(struct stl_thread *thread);
int stl_thread_start(struct stl_thread *thread, void *(*fn)(void *), void *arg);
int stl_thread_join(struct stl_thread *thread, void **ret);
...
unsigned int __stdcall stl_thread_fn(void *arg)
{
    struct stl_thread *thread = arg;
...
int stl_thread_start(struct stl_thread *thread, void *(*fn)(void *), void *arg)
{
    ...
    thread->id =
            (HANDLE) _beginthreadex(NULL, 0, stl_thread_fn, thread, 0, NULL);
    rc = thread->id == 0 ? -1 : 0;
...
int stl_thread_term(struct stl_thread *thread)
{
    return stl_thread_join(thread, NULL);
}

void *thread1_fn(void *arg)
{
    char *data;
    struct stl_cond *cond = arg;

    data = stl_cond_wait(cond);
...
void fail_test()
{
    struct stl_cond cond;

    mock_attrinit = true;
    assert(stl_cond_init(&cond) == -1);
    mock_attrinit = false;
    ...
    assert(stl_cond_init(&cond) == 0);
    assert(stl_cond_term(&cond) == 0);
    mock_mutexinit = true;
    assert(stl_cond_init(&cond) == -1);
...
{
    struct stl_cond cond;
    struct stl_thread thread1;
    struct stl_thread thread2;

    assert(stl_cond_init(&cond) == 0);
    stl_thread_init(&thread1);
    stl_thread_init(&thread2);
    assert(stl_thread_start(&thread1, thread1_fn, &cond) == 0);
    assert(stl_thread_start(&thread2, thread2_fn, &cond) == 0);
    assert(stl_thread_term(&thread1) == 0);
    assert(stl_thread_term(&thread2) == 0);
    assert(stl_cond_term(&cond) == 0);

    assert(stl_cond_init(&cond) == 0);
    stl_thread_init(&thread1);
    stl_thread_init(&thread2);
    assert(stl_thread_start(&thread1, thread1a_fn, &cond) == 0);
    assert(stl_thread_start(&thread2, thread2_fn, &cond) == 0);
    assert(stl_thread_term(&thread1) == 0);
    assert(stl_thread_term(&thread2) == 0);
    assert(stl_cond_term(&cond) == 0);
    ...
    stl_thread_init(&thread2);
    assert(stl_thread_start(&thread1, thread1_fn, &cond) == 0);
    assert(stl_thread_start(&thread2, thread2a_fn, &cond) == 0);
    assert(stl_thread_term(&thread1) == 0);
    assert(stl_thread_term(&thread2) == 0);
    assert(stl_cond_term(&cond) == 0);
}
