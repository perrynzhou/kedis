...
void test1()
{
    ...
    assert(rc == 0);
    rc = stl_mmap_msync(&mmap, 0, 4096);
    assert(rc == 0);
    rc = stl_mmap_term(&mmap);
    assert(rc == 0);
    ...
    *p = 'x';
    ...
    rc = stl_mmap_term(&mmap);
    assert(rc == 0);
    rc = stl_mmap_init(&mmap, "x.txt", O_RDONLY, PROT_READ, MAP_SHARED, 0, 0);
    assert(rc == 0);
    p = mmap.ptr;
    assert(*p == 'x');
    rc = stl_mmap_term(&mmap);
    ...
    rc = stl_mmap_init(&mmap, "x.txt", O_RDWR | O_CREAT | O_TRUNC,
                       PROT_READ | PROT_WRITE, MAP_SHARED, 0, 4095);
    assert(rc == 0);
    rc = stl_mmap_mlock(&mmap, 0, 4095);
    assert(rc == 0);
    rc = stl_mmap_munlock(&mmap, 0, 4095);
    assert(rc == 0);
    rc = stl_mmap_term(&mmap);
    assert(rc == 0);
}
...
bool fail_stat;
extern int __real_stat(const char *pathname, stat *statbuf);
int __wrap_stat(const char *pathname, stat *statbuf)
{
    if (fail_stat)
    {
        return -1;
...
void fail_test()
{
    ...
    fail_munlock = true;
    rc = stl_mmap_munlock(&mmap, 0, 4096);
    assert(rc == -1);
    fail_munlock = false;
    rc = stl_mmap_munlock(&mmap, 0, 4096);
    assert(rc == 0);
